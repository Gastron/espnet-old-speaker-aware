...
train_set=train_si284_ivec
train_dev=test_dev93_ivec
train_test=test_eval92_ivec
recog_set="test_dev93_ivec test_eval92_ivec"
...
if [ ${stage} -le 1 ]; then
    ...
    utils/copy_data_dir.sh data/test_dev93 data/${train_dev}
    utils/copy_data_dir.sh data/test_eval92 data/${train_test}
    cp data/train_si284/cmvn.ark data/train_si284_ivec/
fi
...
if [ ${stage} -le 2 ]; then
    ...
    steps/nnet/ivector/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 --ivector_dim 200 data/${train_set} ${ivec_extractor_dir}/diag_ubm_train ${ivec_extractor_dir}
fi
...
if [ ${stage} -le 3 ]; then
    ...
    local/extract_ivectors.sh --cmd "$train_cmd" --nj 30 \
      data/${train_dev} ${ivec_extractor_dir} data/${train_dev}/ivecs
fi

if [ ${stage} -le 4 ]; then
    for rtask in ${recog_set}; do
      local/extract_ivectors.sh --cmd "$train_cmd" --nj 4 \
...
if [ ${stage} -le 5 ]; then
    ...
    local/dump_with_ivec.sh --cmd "$train_cmd" --nj 4 --do_delta $do_delta \
        data/${train_dev}/feats.scp data/${train_set}/cmvn.ark data/${train_dev}/ivecs exp/dump_feats/dev ${feat_dt_dir}
    for rtask in ${recog_set}; do
