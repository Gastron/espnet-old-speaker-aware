...
if [ $# != 5 ]; then
    echo "Usage: $0 <scp> <cmvnark> <embscp> <logdir> <dumpdir>"
    exit 1;
fi
...
for n in $(seq $nj); do
    utils/filter_scp.pl $logdir/feats.$n.scp $embscp > $logdir/embs.$n.scp
done
...
if ${do_delta};then
    $cmd JOB=1:$nj $logdir/dump_feature.JOB.log \
        ...
        add-deltas ark:- ark:- \| \
        append-vector-to-feats ark:- scp:$logdir/embs.JOB.scp ark:- \| \
