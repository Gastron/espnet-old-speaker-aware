...
if [ $# != 6 ]; then
    echo "Usage: $0 <scp> <cmvnark> <meanvec> <ivecdir> <logdir> <dumpdir>"
    exit 1;
fi
...
meanvec=$3
ivecdir=$4
ivecscp=$ivecdir/ivector.scp #used to be ivector_abs.scp, but changed
logdir=$5
dumpdir=$6
...
if ${do_delta};then
    $cmd JOB=1:$nj $logdir/dump_feature.JOB.log \
        ...
        add-deltas ark:- ark:- \| \
        append-vector-to-feats ark:- "ark:ivector-subtract-global-mean $meanvec scp:$logdir/ivecs.JOB.scp ark:- | ivector-normalize-length --scaleup=false ark:- ark:- |" ark:- \| \
