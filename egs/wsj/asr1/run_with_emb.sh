...
#Embedding Model
emb_model="AkuNet.256.200.2.2000.1000."
...
train_set=train_si284_emb
train_dev=test_dev93_emb
train_test=test_eval92_emb
recog_set="test_dev93_emb test_eval92_emb"
...
if [ ${stage} -le 2 ]; then
    ...
    local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --model $emb_model --nj 4 data/${train_set}
fi

if [ ${stage} -le 3 ]; then
    local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --model $emb_model --nj 1 data/${train_dev}
    for rtask in ${recog_set}; do
      local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --model $emb_model --nj 1 data/${rtask}
    done
fi
...
if [ ${stage} -le 4 ]; then
    ...
    local/dump_with_emb.sh --cmd "$train_cmd" --nj 4 --do_delta $do_delta \
        data/${train_dev}/feats.scp data/${train_set}/cmvn.ark data/${train_dev}/embs.scp exp/dump_feats/dev ${feat_dt_dir}
    for rtask in ${recog_set}; do
