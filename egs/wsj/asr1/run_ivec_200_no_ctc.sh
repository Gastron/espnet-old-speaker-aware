...
train_set=train_si284_ivec_200
dev_set=test_dev93_ivec_200
test_set=test_eval92_ivec_200
recog_set="test_dev93_ivec_200 test_eval92_ivec_200"
...
if [ ${stage} -le 4 ]; then
    ...
    local/dump_with_ivec.sh --cmd "$train_cmd" --nj 4 --do_delta $do_delta \
        data/${dev_set}/feats.scp data/${train_set}/cmvn.ark data/${train_set}/ivectors/mean.vec data/${train_set}/ivectors/lda-transform.mat data/${dev_set}/ivectors exp/dump_feats/${dev_set} ${feat_dt_dir}
    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}; mkdir -p ${feat_recog_dir}
        local/dump_with_ivec.sh --cmd "$train_cmd" --nj 4 --do_delta $do_delta \
            data/${rtask}/feats.scp data/${train_set}/cmvn.ark data/${train_set}/ivectors/mean.vec data/${train_set}/ivectors/lda-transform.mat data/${rtask}/ivectors exp/dump_feats/recog/${rtask} \
            ${feat_recog_dir}
    done
fi
