...
train_set=train_trim_emb_aku
dev_set=dev_trim_emb_aku
recog_set="dev_emb_aku test_emb_aku"
...
if [ ${stage} -le 0 ]; then
    ...
    utils/copy_data_dir.sh data/dev data/dev_emb_aku
    utils/copy_data_dir.sh data/test data/test_emb_aku
    cp data/train_trim/cmvn.ark data/train_trim_emb_aku/
fi
...
if [ ${stage} -le 2 ]; then
    ...
    local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --nj 6 data/${train_set}
fi

if [ ${stage} -le 3 ]; then
    local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --nj 1 data/${dev_set}
    for rtask in ${recog_set}; do
      local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --nj 1 data/${rtask}
    done
fi
if [ ${stage} -le 4 ]; then
