...
train_set=train_trim_emb_ula_vgg
dev_set=dev_trim_emb_ula_vgg
recog_set="dev_emb_ula_vgg test_emb_ula_vgg"
...
if [ ${stage} -le 0 ]; then
    ...
    utils/copy_data_dir.sh data/dev data/dev_emb_ula_vgg
    utils/copy_data_dir.sh data/test data/test_emb_ula_vgg
    cp data/train_trim/cmvn.ark data/train_trim_emb_ula_vgg/
fi
...
if [ ${stage} -le 2 ]; then
    ...
    local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --nj 6 \
      --embmodel "xie_nagrani/weights.h5" \
      --extract_script local/extract_ula_vgg_embedding.py \
      data/${train_set}
fi
