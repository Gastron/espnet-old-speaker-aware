...
embmodel="SS_200_woL2.1.200.2000"
...
train_set=train_trim_emb_wol2
dev_set=dev_trim_emb_wol2
recog_set="dev_emb_wol2 test_emb_wol2"
...
if [ ${stage} -le 0 ]; then
    ...
    utils/copy_data_dir.sh data/dev data/dev_emb_wol2
    utils/copy_data_dir.sh data/test data/test_emb_wol2
    cp data/train_trim/cmvn.ark data/train_trim_emb_wol2/
fi
...
if [ ${stage} -le 2 ]; then
    ...
    local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --nj 6 \
      --embmodel "$embmodel" --extra_opts "--no-l2-norm" data/${train_set}
fi

if [ ${stage} -le 3 ]; then
    local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --nj 1 \
      --embmodel "$embmodel" --extra_opts "--no-l2-norm" data/${dev_set}
    for rtask in ${recog_set}; do
      local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --nj 1 \
        --embmodel "$embmodel" --extra_opts "--no-l2-norm" data/${rtask}
    done
fi
if [ ${stage} -le 4 ]; then
