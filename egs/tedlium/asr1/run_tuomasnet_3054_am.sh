...
embmodel="first_big_triton_test.3054.AM.30.0.35"
...
train_set=train_trim_emb_tuomasnet_3054_am
dev_set=dev_trim_emb_tuomasnet_3054_am
recog_set="dev_emb_tuomasnet_3054_am test_emb_tuomasnet_3054_am"
...
if [ ${stage} -le 0 ]; then
    ...
    utils/copy_data_dir.sh data/dev data/dev_emb_tuomasnet_3054_am
    utils/copy_data_dir.sh data/test data/test_emb_tuomasnet_3054_am
    cp data/train_trim/cmvn.ark data/train_trim_emb_tuomasnet_3054_am/
fi
...
if [ ${stage} -le 2 ]; then
    ...
    local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --nj 6 \
      --embmodel "$embmodel" data/${train_set}
fi

if [ ${stage} -le 3 ]; then
    local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --nj 1 \
      --embmodel "$embmodel" data/${dev_set}
    for rtask in ${recog_set}; do
      local/compute_speaker_embeddings.sh --cmd "$emb_cmd" --nj 1 \
        --embmodel "$embmodel" data/${rtask}
    done
fi
if [ ${stage} -le 4 ]; then
