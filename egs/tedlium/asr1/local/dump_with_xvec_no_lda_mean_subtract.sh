...
if [ $# != 6 ]; then
    echo "Usage: $0 <feats.scp> <cmvn-ark> <meanvec> <xvecdir> <logdir> <dumpdir>"
    exit 1;
fi
...
if ${do_delta};then
    $cmd JOB=1:$nj $logdir/dump_feature.JOB.log \
        ...
        add-deltas ark:- ark:- \| \
        append-vector-to-feats ark:- "ark:ivector-subtract-global-mean $meanvec scp:$logdir/xvecs.JOB.scp ark:- | ivector-normalize-length --scaleup=$scaleup ark:- ark:- |" ark:- \| \
